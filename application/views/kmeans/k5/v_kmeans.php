<section class="home-section">
    ...
    <div class="container-fluid">
      ...
      <a class="btn btn-primary" href="<?php echo base_url(); ?>kmeans/iterasi_kmeans_k5_lanjut">Proses Iterasi Selanjutnya</a>
      <br><br>

      <?php
      $this->db->query('truncate table centroid_k5_awal');

      foreach($c1->result() as $value_ci1){
        $data_c1 = $value_ci1->idmalaria_baru;
      }

      foreach($c2->result() as $value_ci2){
        $data_c2 = $value_ci2->idmalaria_baru;
      }

      foreach($c3->result() as $value_ci3){
        $data_c3 = $value_ci3->idmalaria_baru;
      }

      foreach($c4->result() as $value_ci4){
        $data_c4 = $value_ci4->idmalaria_baru;
      }

      foreach($c5->result() as $value_ci5){
        $data_c5 = $value_ci5->idmalaria_baru;
      }

      $q = "insert into centroid_k5_awal(idmalaria_c1, idmalaria_c2, idmalaria_c3, idmalaria_c4, idmalaria_c5) values('".$data_c1."','".$data_c2."','".$data_c3."','".$data_c4."','".$data_c5."')";
      $this->db->query($q);

      // data centroid 1
      $this->db->where('idmalaria_c1', $data_c1);
      $this->db->join('data_malaria_baru', 'data_malaria_baru.idmalaria_baru = centroid_k5_awal.idmalaria_c1');
      $data_centroid1 = $this->db->get('centroid_k5_awal');
      ...
      // data centroid 2
      $this->db->where('idmalaria_c2', $data_c2);
      $this->db->join('data_malaria_baru', 'data_malaria_baru.idmalaria_baru = centroid_k5_awal.idmalaria_c2');
      $data_centroid2 = $this->db->get('centroid_k5_awal');
      ...
      // data centroid 3
      $this->db->where('idmalaria_c3', $data_c3);
      $this->db->join('data_malaria_baru', 'data_malaria_baru.idmalaria_baru = centroid_k5_awal.idmalaria_c3');
      $data_centroid3 = $this->db->get('centroid_k5_awal');
      ...
      // data centroid 4
      $this->db->where('idmalaria_c4', $data_c4);
      $this->db->join('data_malaria_baru', 'data_malaria_baru.idmalaria_baru = centroid_k5_awal.idmalaria_c4');
      $data_centroid4 = $this->db->get('centroid_k5_awal');
      ...
      // data centroid 5
      $this->db->where('idmalaria_c5', $data_c5);
      $this->db->join('data_malaria_baru', 'data_malaria_baru.idmalaria_baru = centroid_k5_awal.idmalaria_c5');
      $data_centroid5 = $this->db->get('centroid_k5_awal');
          ...
          <table class="table table-striped data">
              <thead>
                ...
                <?php
                  ...
                  $this->db->query('truncate table centroid_temp_baru');
                  $this->db->query('truncate table hasil_centroid_baru');
                ?>
              </thead>
            <tbody>
            <?php foreach ($cluster->result_array() as $s) { ?>
                ...
                $q = "insert into centroid_temp_baru(iterasi,idmalaria_baru,c1,c2,c3,c4,c5) values(1,'".$s['idmalaria_baru']."','".$arr_c1[$no]."','".$arr_c2[$no]."','".$arr_c3[$no]."','".$arr_c4[$no]."','".$arr_c5[$no]."')";
                $this->db->query($q);
                ...
                $this->db->where('c1', 1);
                $this->db->join('data_malaria_baru','data_malaria_baru.idmalaria_baru = centroid_temp_baru.idmalaria_baru');
                $centroid_temp = $this->db->get('centroid_temp_baru')->result();
                ...
                $this->db->where('c2', 1);
                $this->db->join('data_malaria_baru','data_malaria_baru.idmalaria_baru = centroid_temp_baru.idmalaria_baru');
                $centroid_temp_c2 = $this->db->get('centroid_temp_baru')->result();
                ...
                $this->db->where('c3', 1);
                $this->db->join('data_malaria_baru','data_malaria_baru.idmalaria_baru = centroid_temp_baru.idmalaria_baru');
                $centroid_temp_c3 = $this->db->get('centroid_temp_baru')->result();
                ...
                $this->db->where('c4', 1);
                $this->db->join('data_malaria_baru','data_malaria_baru.idmalaria_baru = centroid_temp_baru.idmalaria_baru');
                $centroid_temp_c4 = $this->db->get('centroid_temp_baru')->result();
                ...
                $this->db->where('c5', 1);
                $this->db->join('data_malaria_baru','data_malaria_baru.idmalaria_baru = centroid_temp_baru.idmalaria_baru');
                $centroid_temp_c5 = $this->db->get('centroid_temp_baru')->result();
                ...
                $q = "insert into hasil_centroid_baru(c1a,c1b,c1c,c1d,c1e,c1f,c1g,c1h,c1i,c1j,c1k,c1l,c1m,c2a,c2b,c2c,c2d,c2e,c2f,c2g,c2h,c2i,c2j,c2k,c2l,c2m,c3a,c3b,c3c,c3d,c3e,c3f,c3g,c3h,c3i,c3j,c3k,c3l,c3m,c4a,c4b,c4c,c4d,c4e,c4f,c4g,c4h,c4i,c4j,c4k,c4l,c4m,c5a,c5b,c5c,c5d,c5e,c5f,c5g,c5h,c5i,c5j,c5k,c5l,c5m) values('".$c1a_b."','".$c1b_b."','".$c1c_b."','".$c1d_b."','".$c1e_b."','".$c1f_b."','".$c1g_b."','".$c1h_b."','".$c1i_b."','".$c1j_b."','".$c1k_b."','".$c1l_b."','".$c1m_b."','".$c2a_b."','".$c2b_b."','".$c2c_b."','".$c2d_b."','".$c2e_b."','".$c2f_b."','".$c2g_b."','".$c2h_b."','".$c2i_b."','".$c2j_b."','".$c2k_b."','".$c2l_b."','".$c2m_b."','".$c3a_b."','".$c3b_b."','".$c3c_b."','".$c3d_b."','".$c3e_b."','".$c3f_b."','".$c3g_b."','".$c3h_b."','".$c3i_b."','".$c3j_b."','".$c3k_b."','".$c3l_b."','".$c3m_b."','".$c4a_b."','".$c4b_b."','".$c4c_b."','".$c4d_b."','".$c4e_b."','".$c4f_b."','".$c4g_b."','".$c4h_b."','".$c4i_b."','".$c4j_b."','".$c4k_b."','".$c4l_b."','".$c4m_b."','".$c5a_b."','".$c5b_b."','".$c5c_b."','".$c5d_b."','".$c5e_b."','".$c5f_b."','".$c5g_b."','".$c5h_b."','".$c5i_b."','".$c5j_b."','".$c5k_b."','".$c5l_b."','".$c5m_b."')";
                $this->db->query($q);
